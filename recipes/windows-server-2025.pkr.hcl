...
locals {
  ...
  ps_execute = "powershell -executionpolicy bypass \"& { $ErrorActionPreference='Stop'; $_p='{{.Path}}'; $_v='{{.Vars}}'; $_isCleanup=[IO.Path]::GetFileName($_p) -like 'packer-cleanup-*'; $_needsVars=-not $_isCleanup; $_dl=[DateTime]::Now.AddSeconds(300); while ((-not (Test-Path $_p) -or ($_needsVars -and -not (Test-Path $_v))) -and [DateTime]::Now -lt $_dl) { Start-Sleep 2 }; if (-not (Test-Path $_p)) { Write-Host ('PROVISIONER ERROR: script never arrived at ' + $_p + ' - the upload did not land within 300s'); exit 1 }; if ($_needsVars -and -not (Test-Path $_v)) { Write-Host ('PROVISIONER ERROR: env-vars file never arrived at ' + $_v + ' - the upload did not land within 300s'); exit 1 }; try { if (Test-Path $_v) { . $_v }; & $_p } catch { $_line=$_.InvocationInfo.ScriptLineNumber; $_source=[string]$_.InvocationInfo.Line; Write-Host ('PROVISIONER ERROR at line ' + $_line + ' (' + $_source.Trim() + '): ' + $_.Exception.Message); exit 1 }; if ($LastExitCode) { exit $LastExitCode }; exit 0 }\""
